...
public class EternalFireBlock extends Block
{
        ...
        if(world.getGameRules().getBoolean(GameRules.DO_FIRE_TICK))
        {
            ...
            else
            {
                ...
                if(!isFireSource)
                {
                    ...
                    {
                        if(!world.getBlockState(downPos).func_224755_d(world, downPos, Direction.UP) || age > 3)
                        {
                            world.removeBlock(pos, false);
                        }
                        return;
    ...
    protected void tryCatchFire(World world, BlockPos pos, int chance, Random random, int age, Direction direction)
    {
        ...
        int flammability = world.getBlockState(targetPos).getFlammability(world, targetPos, direction);
        ...
        if(random.nextInt(chance) < flammability)
        {
            ...
            if(random.nextInt(age + 10) < 5)
            {
                ...
                world.setBlockState(targetPos, this.getStateForPlacement(world, targetPos).with(AGE, newAge), 3);
            }
            else
            {
                if(age > 0)
